...
def make_cifar10_dirichlet(
    ...
    download=True,
    normalize_0_1=True,
    flatten= False            # True -> (n, 3072), False -> (n, 32, 32, 3)
):
    """
    1) Load CIFAR-10 train+test, combine into one pool
    ...
    Returns (ragged per client, dtype=object):
      ...
      x_* shapes: (n_i, 3072) if flatten else (n_i, 32, 32, 3)
    """
    # Load original train/test
    tr = datasets.CIFAR10(root=root, train=True, download=download)
    te = datasets.CIFAR10(root=root, train=False, download=download)

    X_all = np.concatenate([tr.data, te.data], axis=0)  # (60000, 32, 32, 3) uint8
    y_all = np.asarray(tr.targets + te.targets, dtype=np.int64)  # (60000,)

    if normalize_0_1:
        X_all = X_all.astype(np.float32) / 255.0

    if flatten:
        X_all = X_all.reshape(len(X_all), -1)  # -> (60000, 3072)
    ...
    # Build per-client train/test arrays
...
if __name__ == "__main__":
    ...
    x_train, y_train, x_test, y_test = make_cifar10_dirichlet(
        n_clients= num_clients, alpha=alpha, seed=1, train_ratio=0.8, flatten=False,
        min_train=200, min_test=50
    )
